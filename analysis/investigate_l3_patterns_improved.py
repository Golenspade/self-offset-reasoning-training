...
class L3PatternAnalyzer:
        ...
        if target_match:
            ...
            for pattern in noise_patterns:
                ...
                if noise_match:
                    ...
                    # 检查直接映射关系
                    if self._normalize_expression(noise_a) == self._normalize_expression(target_a) and \
                       self._normalize_expression(noise_b) == self._normalize_expression(target_b):
                        results.update({
    ...
    def run_comprehensive_analysis(self, sample_size: int = 50) -> Dict[str, any]:
        ...
        # 随机采样
        analysis_samples = random.sample(self.samples, min(sample_size, len(self.samples)))
        ...
        for i, sample in enumerate(analysis_samples):
            result = self.check_simple_transformations_robust(
                sample.get('noisy_prop', ''),
                sample.get('target_contrapositive', '')
            )
        ...
        # 2. 噪声有效性分析
        noise_analysis = self.analyze_noise_effectiveness_comprehensive(analysis_samples)
        ...
        self.analysis_results = {
            ...
            'overall_risk_score': self._calculate_risk_score(cheating_patterns, noise_analysis)
        }
    ...
    def _calculate_risk_score(self, cheating_patterns: List[Dict], noise_analysis: Dict) -> float:
        """计算整体风险分数"""
        risk_score = 0.0
        
        # 作弊模式风险
        if cheating_patterns:
            high_confidence_patterns = [p for p in cheating_patterns if p['confidence'] > 0.9]
        ...
        if noise_analysis['total_samples'] > 0:
            multi_noise_ratio = noise_analysis['multi_noise_samples'] / noise_analysis['total_samples']
...
def main():
    ...
    if results:
        ...
        # 保存结果
        with open('outputs/l3_pattern_analysis_improved.json', 'w', encoding='utf-8') as f:
