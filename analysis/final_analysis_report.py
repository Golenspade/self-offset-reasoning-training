...
def create_final_comparison():
    ...
    bars1 = ax1.bar(
        x - width / 2, before_accs, width, label="修复前", alpha=0.7, color="red"
    )
    bars2 = ax1.bar(
        x + width / 2, after_accs, width, label="修复后", alpha=0.7, color="green"
    )
    ...
    bars3 = ax2.bar(
        x - width / 2, before_losses, width, label="修复前", alpha=0.7, color="red"
    )
    bars4 = ax2.bar(
        x + width / 2, after_losses, width, label="修复后", alpha=0.7, color="green"
    )
    ...
    ax3.plot(
        epochs, level1_curve, "o-", color="blue", linewidth=2, label="Level 1 (修复后)"
    )
    ax3.plot(
        epochs, level2_curve, "s-", color="green", linewidth=2, label="Level 2 (修复后)"
    )
    ax3.plot(
        epochs, level3_curve, "^-", color="red", linewidth=2, label="Level 3 (修复后)"
    )
...
修复取得了重大进展，2/3的问题已解决！
    ...
    plt.savefig(
        "outputs/figures/fix_comparison_analysis.png", dpi=300, bbox_inches="tight"
    )
