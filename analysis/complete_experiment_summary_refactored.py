...
class ExperimentAnalyzer:
    ...
    def normalize_metric(
        self, value: float, metric_name: str, invert: bool = False
    ) -> float:
        """
        动态归一化指标值
...
def create_detailed_analysis_plot(analyzer: ExperimentAnalyzer):
    ...
    # 1. 逻辑准确率对比
    logical_accuracies = [
        experiments_data[name]["final_logical_accuracy"] for name in names
    ]
    ...
    scatter = ax3.scatter(
        training_samples, logical_accuracies, c=colors[: len(names)], s=100, alpha=0.7
    )
    ...
    for name in names:
        ...
        loss_score = analyzer.normalize_metric(
            data["final_loss"], "final_loss", invert=True
        )
        stability_score = analyzer.normalize_metric(
            data["stability_score"], "stability_score"
        )
        efficiency_score = analyzer.normalize_metric(
            data["training_samples"], "training_samples", invert=True
        )

        radar_data.append(
            [logical_score, loss_score, stability_score, efficiency_score]
        )
...
def print_comprehensive_summary(analyzer: ExperimentAnalyzer):
    ...
    print(f"\n🎯 关键指标对比:")
    print(
        f"{'实验名称':<20} {'逻辑准确率':<12} {'最终损失':<12} {'稳定性':<10} {'样本数':<8}"
    )
    print("-" * 70)
    ...
    for name, data in experiments_data.items():
        ...
        print(
            f"{short_name:<20} {data['final_logical_accuracy']:<12.3f} "
    ...
    # 找出最佳表现
    best_logical = max(
        experiments_data.items(), key=lambda x: x[1]["final_logical_accuracy"]
    )
    best_loss = min(experiments_data.items(), key=lambda x: x[1]["final_loss"])
    best_stability = max(
        experiments_data.items(), key=lambda x: x[1]["stability_score"]
    )

    print(
        f"  📈 最高逻辑准确率: {best_logical[0]} ({best_logical[1]['final_logical_accuracy']:.3f})"
    )
    print(f"  📉 最低损失: {best_loss[0]} ({best_loss[1]['final_loss']:.4f})")
    print(
        f"  🎯 最高稳定性: {best_stability[0]} ({best_stability[1]['stability_score']:.3f})"
    )
    ...
    logical_accuracies = [
        data["final_logical_accuracy"] for data in experiments_data.values()
    ]
