...
def create_training_summary():
    ...
    train_loss = [1.4242, 0.9924, 0.9100, 0.8706, 0.8447, 0.8261, 0.8118, 0.8003, 0.7912, 0.7831]
    ...
    fig.suptitle('前10轮正式训练结果 - 自偏移推理训练初步成果', fontsize=16, fontweight='bold')
    ...
    ax1.text(5, 1.2, f'损失下降: {train_loss[0]:.3f} → {train_loss[-1]:.3f}', 
             fontsize=12, bbox=dict(boxstyle="round,pad=0.3", facecolor="lightblue"))
    
    # 2. 准确率对比
    ax2.plot(epochs, logical_acc, 'g-', linewidth=3, marker='^', markersize=6, label='神经网络逻辑准确率')
    ax2.plot(epochs, hybrid_acc, 'purple', linewidth=3, marker='D', markersize=6, label='混合系统准确率')
    ax2.set_title('准确率对比', fontsize=14, fontweight='bold')
    ax2.set_xlabel('训练轮次')
    ax2.set_ylabel('准确率 (%)')
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    ax2.text(6, 75, '混合系统稳定在36%', fontsize=12, fontweight='bold',
...
🚀 下一步优化方向
   ...
   • 扩展到更复杂的逻辑推理
    ...
    plt.savefig('outputs/formal_training/figures/10_epochs_summary.png', dpi=300, bbox_inches='tight')
    plt.savefig('outputs/formal_training/figures/10_epochs_summary.pdf', bbox_inches='tight')
