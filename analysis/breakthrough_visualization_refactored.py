...
def validate_data_completeness(history):
    ...
    Returns:
        dict: 验证结果和统计信息
    """
    validation_result = {"is_valid": True, "warnings": [], "stats": {}}
    ...
    if all(field in history for field in required_fields):
        ...
        if len(set(lengths)) > 1:
            validation_result["warnings"].append(
                f"数据长度不一致: {dict(zip(required_fields, lengths))}"
            )
...
def create_breakthrough_visualization():
    ...
    fig.suptitle(
        "📊 突破性训练系统 - 真实数据可视化报告", fontsize=20, fontweight="bold", y=0.98
    )
    ...
    # 获取最终指标 (如果有数据的话)
    if metrics_over_time and any(
        len(values) > 0 for values in metrics_over_time.values()
    ):
        # 只使用真实的、有意义的指标
        categories = []
        values = []
...
def main():
    ...
    try:
        ...
        print("\n🎉 可视化生成完成!")
        print(
            "📁 输出文件: outputs/breakthrough_training/breakthrough_visualization_honest.png"
        )
