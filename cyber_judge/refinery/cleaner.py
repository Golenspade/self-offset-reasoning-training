...
class DataCleaner:
    ...
    def clean_judgment(self, raw_judgment: Dict) -> Optional[CleanedJudgment]:
        """清洗单条判例"""
        # 兼容不同的数据格式
        case = self.clean_text(
            raw_judgment.get("case") or raw_judgment.get("title", "")
        )
        verdict = self.clean_text(
            raw_judgment.get("verdict") or raw_judgment.get("content", "")
        )
    ...
    def clean_dataset(
        self, input_file: Path, output_file: Path, min_quality: float = 0.5
    ) -> List[CleanedJudgment]:
        """清洗整个数据集"""
        ...
        with open(output_file, "w", encoding="utf-8") as f:
            json.dump(
                [asdict(j) for j in cleaned_data], f, ensure_ascii=False, indent=2
            )
...
def main():
    ...
    cleaner.clean_dataset(
        input_file, output_file, min_quality=0.1
    )  # 降低阈值以接受更多数据
