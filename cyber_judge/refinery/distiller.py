...
"""
    ...
    async def distill_batch(self, data: List[Dict], 
                           max_concurrent: int = 5) -> List[DistilledExample]:
        """批量蒸馏数据"""
        ...
        async def distill_with_limit(item):
            async with semaphore:
                return await self.distill_single(
                    item['instruction'], 
                    item['output']
                )
    ...
    def save_for_production(self, examples: List[DistilledExample], 
                           output_file: Path):
    ...
    def save_for_training(self, examples: List[DistilledExample], 
                         output_file: Path):
        """保存为训练格式 (JSONL)"""
        output_file.parent.mkdir(parents=True, exist_ok=True)
...
async def main():
    ...
    # 读取清洗后的数据
    input_file = Path(__file__).parent.parent / 'data' / 'processed' / 'cleaned_judgments.json'
