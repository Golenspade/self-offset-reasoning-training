...
class ReActAgent:
    ...
    def _parse_thought(self, llm_output: str) -> Thought:
        ...
        thought_match = re.search(
            r"Thought:\s*(.+?)(?=Action:|$)", llm_output, re.DOTALL
        )
        ...
        return Thought(content=thought, action=action, action_input=action_input)
    ...
    async def _search_web(self, query: str) -> Observation:
        """搜索网络（占位符）"""
        # TODO: 实现实际的网络搜索
        return Observation(content=f"网络搜索结果: {query}（功能开发中）", source="web")
    ...
    async def think_and_act(
        self, user_message: str, user_id: str, context: str = ""
    ) -> str:
        """
        思考并行动
        ...
        for iteration in range(self.max_iterations):
            ...
            prompt = self._build_react_prompt(
                user_message, user_id, context, conversation_history
            )

            # 调用 LLM 思考
            llm_output = await self.llm.chat_with_system_prompt(
                user_message=prompt, system_prompt=self._get_react_system_prompt()
            )
            ...
            observation = await self._execute_action(
                thought.action, user_id, thought.action_input
            )
...
请直接给出判决，不要再思考。"""
    ...
    async def _execute_action(
        self, action: ActionType, user_id: str, action_input: str
    ) -> Observation:
        """执行行动"""
    ...
    def _build_react_prompt(
        self, user_message: str, user_id: str, context: str, history: List[str]
    ) -> str:
        """构建 ReAct 提示词"""
...
async def example_usage():
    ...
    response = await agent.think_and_act(
        user_message="4060 能跑 AI 吗？", user_id="user_123", context="群聊讨论 AI 训练"
    )
