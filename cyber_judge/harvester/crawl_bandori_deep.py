    ...
    try:
        ...
        for i, reply_elem in enumerate(reply_elems[:20]):  # 只取前20条回复
            try:
                ...
                replies.append(
                    {"author": author.strip(), "content": content, "floor": i + 1}
                )

            except Exception as e:
                continue

        return {"op_content": op_content.strip(), "replies": replies}
...
async def extract_threads_from_page(page, context, page_num, crawl_replies=True):
    ...
    for i, thread in enumerate(threads[:15]):  # 每页取15个
        try:
            ...
            reply_count_int = (
                int(reply_count.strip()) if reply_count.strip().isdigit() else 0
            )
...
async def crawl_bandori_tieba(max_pages=5):
    ...
    async with async_playwright() as p:
        ...
        for page_num in range(1, max_pages + 1):
            ...
            judgments = await extract_threads_from_page(
                page, context, page_num, crawl_replies=True
            )
