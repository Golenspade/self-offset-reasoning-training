...
class TiebaCrawler:
    ...
    async def crawl_tieba_page(
        self, page: Page, tieba_name: str, page_num: int = 0
    ) -> List[Dict]:
        """爬取单个贴吧页面"""
        ...
        try:
            ...
            for thread in threads[:10]:  # 每页只取前10个
                try:
                    ...
                    author = (
                        await author_elem.inner_text() if author_elem else "unknown"
                    )
                    author = author.strip()
    ...
    async def crawl_multiple_tiebas(
        self, tieba_list: List[str], pages_per_tieba: int = 2
    ):
        """爬取多个贴吧"""
