    ...
    async with async_playwright() as p:
        ...
        if '帖子列表项' in results:
            ...
            if threads:
                ...
                # 标题
                if '帖子标题' in results:
                    title_elem = await first_thread.query_selector(results['帖子标题']['selector'])
                    if title_elem:
                ...
                # 作者
                if '作者' in results:
                    author_elem = await first_thread.query_selector(results['作者']['selector'])
                    if author_elem:
                        data['author'] = await author_elem.inner_text()
                
                # 回复数
                if '回复数' in results:
                    reply_elem = await first_thread.query_selector(results['回复数']['selector'])
                    if reply_elem:
