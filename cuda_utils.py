...
class CUDAManager:
    ...
    def get_best_device(self):
        ...
        for i in range(device_count):
            ...
            logger.info(
                f"  内存: {memory_total/1e9:.1f}GB (可用: {memory_free/1e9:.1f}GB)"
            )
    ...
    def optimize_cuda_settings(self):
        ...
        if (
            not TORCH_AVAILABLE
            or (hasattr(self.device, "type") and self.device.type != "cuda")
            or (isinstance(self.device, str) and not self.device.startswith("cuda"))
        ):
            return
        ...
        try:
            ...
            if hasattr(torch.cuda, "set_per_process_memory_fraction"):
                torch.cuda.set_per_process_memory_fraction(
                    self.memory_fraction, self.device
                )
    ...
    def memory_monitor(self, operation_name: str = "操作"):
        ...
        try:
            ...
            logger.info(
                f"  内存增量: {end_memory['allocated_memory'] - start_memory['allocated_memory']:.2f}GB"
            )
    ...
    def check_memory_available(self, required_gb: float) -> bool:
        ...
        if available_gb >= required_gb:
            logger.info(
                f"✅ GPU内存充足: 需要{required_gb:.1f}GB, 可用{available_gb:.1f}GB"
            )
            return True
        else:
            logger.warning(
                f"⚠️ GPU内存不足: 需要{required_gb:.1f}GB, 可用{available_gb:.1f}GB"
            )
            return False

    def get_optimal_batch_size(
        self, model_memory_gb: float, max_batch_size: int = 128
    ) -> int:
        """根据GPU内存自动计算最优批次大小"""
        if self.device.type != "cuda":
        ...
        logger.info(
            f"🎯 推荐批次大小: {optimal_batch_size} (基于{available_memory:.1f}GB可用内存)"
        )
    ...
    def supports_mixed_precision(self) -> bool:
        ...
        if supports_fp16:
            ...
            logger.info(
                f"⚠️ 不支持混合精度训练 (计算能力: {props.major}.{props.minor}, 需要7.0+)"
            )
    ...
    def __str__(self) -> str:
        ...
        memory_info = self.get_memory_info()
        return (
            f"CUDAManager(device={self.device}, "
        ...
        if info["cuda_available"]:
            ...
            for gpu in gpu_list:
                if "error" not in gpu:
                    ...
                    print(
                        f"  内存: {gpu['allocated_memory']:.1f}GB / {gpu['total_memory']:.1f}GB"
                    )
