...
class CleanEvaluationSystem:
    ...
    def load_model(self, model_path: str) -> bool:
        ...
        try:
            self.model = ImprovedSimpleModel(
                vocab_size=self.tokenizer.vocab_size, hidden_size=128, max_length=50
            )
    ...
    def evaluate_model_performance(
        self, test_data: List[Dict], max_samples: int = 100
    ) -> Dict:
        """评估模型性能 - 直接、有效的评估"""
        if not self.model:
            print("❌ 请先加载模型")
        ...
        print(f"\n📊 评估结果:")
        print(
            f"  精确匹配准确率: {exact_accuracy:.2%} ({exact_correct}/{total_samples})"
        )
        print(
            f"  逻辑等价准确率: {logical_accuracy:.2%} ({logical_correct}/{total_samples})"
        )

        return results

    def compare_with_real_baseline(
        self, test_data: List[Dict], max_samples: int = 100
    ) -> Dict:
        ...
        print(f"📊 基线结果:")
        print(
            f"  随机预测准确率: {random_accuracy:.2%} ({random_correct}/{total_samples})"
        )
        print(
            f"  规则方法准确率: {rule_accuracy:.2%} ({rule_correct}/{rule_attempted})"
        )
    ...
    def comprehensive_evaluation(self, model_path: str, test_data_path: str) -> Dict:
        ...
        comprehensive_results = {
            ...
            "summary": {
                "model_vs_random": (
                    model_results["logical_accuracy"]
                    / baseline_results["random_accuracy"]
                    if baseline_results["random_accuracy"] > 0
                    else float("inf")
                ),
                "model_vs_rule": (
                    model_results["logical_accuracy"]
                    / baseline_results["rule_accuracy"]
                    if baseline_results["rule_accuracy"] > 0
                    else float("inf")
                ),
        ...
        print(
            f"  相比随机提升: {comprehensive_results['summary']['improvement_over_random']:.1f}个百分点"
        )
        print(
            f"  相比规则提升: {comprehensive_results['summary']['improvement_over_rule']:.1f}个百分点"
        )
