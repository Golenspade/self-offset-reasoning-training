...
def optimize_batch_size(
    cuda_manager: CUDAManager, initial_batch_size: int, model_size_estimate: float = 0.1
) -> int:
    """
    根据GPU内存自动优化批次大小
    ...
    optimal_batch_size = cuda_manager.get_optimal_batch_size(
        model_memory_gb=model_size_estimate, max_batch_size=initial_batch_size * 2
    )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default="outputs/cuda_training", help="输出目录"
    )
    ...
    parser.add_argument(
        "--dim-feedforward", type=int, default=1024, help="前馈网络维度"
    )
    ...
    # CUDA参数
    parser.add_argument(
        "--use-mixed-precision", action="store_true", default=True, help="使用混合精度"
    )
    parser.add_argument(
        "--gpu-memory-fraction", type=float, default=0.8, help="GPU内存使用比例"
    )
    parser.add_argument(
        "--auto-batch-size", action="store_true", help="自动优化批次大小"
    )
    ...
    # 早停参数
    parser.add_argument(
        "--early-stopping-patience", type=int, default=15, help="早停耐心值"
    )
    ...
    cuda_manager = CUDAManager(
        memory_fraction=args.gpu_memory_fraction, auto_optimize=True
    )
    ...
    if args.auto_batch_size:
        config["batch_size"] = optimize_batch_size(
            cuda_manager,
            config["batch_size"],
            model_size_estimate=config["hidden_size"]
    ...
    try:
        ...
        results = trainer.run_cuda_training(
            train_data=train_data, val_data=val_data, output_dir=args.output_dir
        )
