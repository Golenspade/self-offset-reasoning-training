...
class ExperienceReplayBuffer:
    ...
    def sample(self, batch_size: int) -> List[Dict]:
        ...
        indices = np.random.choice(
            len(self.buffer), size=batch_size, p=probabilities, replace=False
        )
...
class BreakthroughTrainingSystem:
    ...
    def __init__(self, config: Dict):
        ...
        self.gradient_clip_threshold = training_config.get(
    ...
    def prepare_training_data(
        self, all_data: List[Dict], epoch: int
        ...
        # 新数据：从可用数据中随机采样
        new_data_size = min(
            len(available_data) // 4, 100
        )  # 每次使用25%的可用数据，最多100个
        new_data = (
            random.sample(available_data, new_data_size) if available_data else []
        )

        # 回放数据：从经验缓冲区采样
        replay_data_size = min(
            len(self.replay_buffer), new_data_size * 2
        )  # 回放数据是新数据的2倍
        replay_data = (
            self.replay_buffer.sample(replay_data_size) if replay_data_size > 0 else []
        )
        ...
        logger.debug(
            f"Epoch {epoch}: 新数据 {len(new_data)}, 回放数据 {len(replay_data)}, 缓冲区利用率 {self.replay_buffer.utilization():.2f}"
        )
    ...
    def train_step(self, sample: Dict) -> Tuple[float, bool]:
        ...
        try:
            ...
            loss = self.model.train_step_improved(
                input_tokens, target_tokens, self.tokenizer
            )
    ...
    def evaluate_validation(self, val_data: List[Dict]) -> float:
        ...
        logger.debug(
            f"验证完成: {successful_evals}/{len(val_data)} 样本成功, 平均损失: {avg_val_loss:.4f}"
        )
            ...
            for epoch in range(epochs):
                ...
                if (epoch + 1) % save_frequency == 0:
                    checkpoint_path = os.path.join(
                        output_dir, f"checkpoint_epoch_{epoch + 1}.npz"
                    )
    ...
    def _log_epoch_results(self, metrics: Dict):
        ...
        logger.info(
            f"  新样本/回放样本: {metrics['new_samples']}/{metrics['replay_samples']}"
        )
...
def create_breakthrough_config() -> Dict:
    ...
    """
    return {
        "model": {"hidden_size": 128, "max_length": 100},
        "training": {
            "initial_lr": 0.001,
        ...
        "precision": {"lr_patience": 3, "lr_decay_factor": 0.7, "min_lr": 1e-6},
        "replay": {"buffer_size": 15000, "initial_fill_ratio": 0.1},
        "curriculum": {
        ...
        if os.path.exists(train_path) and os.path.exists(val_path):
            try:
                ...
                if train_data and val_data:
                    logger.info(
                        f"✅ 数据加载成功: 训练 {len(train_data)}, 验证 {len(val_data)}"
                    )
                    return train_data, val_data
                else:
                    logger.warning(
                        f"⚠️ 数据文件为空或格式错误: {train_file}, {val_file}"
                    )
                    continue
