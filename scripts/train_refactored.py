...
def train_model(model, train_data, val_data, tokenizer, config):
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, len(train_data), batch_size):
            ...
            for sample in batch:
                loss = model.train_step_improved(
                    sample["input"], sample["target"], tokenizer
                )
                batch_loss += loss
        ...
        print(
            ...
            else f"损失={avg_loss:.4f}, " + f"时间={epoch_time:.1f}s"
        )
...
def main():
    ...
    train_data = load_dataset(
        str(train_path), tokenizer, data_config["max_train_samples"]
    )
