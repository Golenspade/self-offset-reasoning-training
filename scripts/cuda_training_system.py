...
class CUDABreakthroughTraining:
    ...
    def __init__(self, config: Dict):
        ...
        self.cuda_manager = CUDAManager(
            memory_fraction=config.get("gpu_memory_fraction", 0.8), auto_optimize=True
        )
        ...
        # 混合精度训练
        self.use_amp = (
            config.get("use_mixed_precision", True)
            and self.device.type == "cuda"
            and self.cuda_manager.supports_mixed_precision()
        )
    ...
    def prepare_batch_cuda(
        ...
        # 计算最大长度
        max_src_len = max(
            len(self.tokenizer.encode(item["noisy_prop"])) for item in batch_data
        )
        max_tgt_len = max(
            len(self.tokenizer.encode(item["target_contrapositive"]))
        ...
        for i, item in enumerate(batch_data):
            ...
            tgt_tokens = [self.tokenizer.START_TOKEN] + self.tokenizer.encode(
                item["target_contrapositive"]
            )
    ...
    def create_masks(
        ...
        # 源序列掩码（padding掩码）
        src_mask = (src == self.tokenizer.PAD_TOKEN).transpose(
            0, 1
        )  # [batch_size, seq_len]

        # 目标序列掩码（因果掩码 + padding掩码）
        tgt_mask = torch.triu(
            torch.ones(tgt_seq_len, tgt_seq_len, device=self.device), diagonal=1
        ).bool()
    ...
    def train_step_cuda(
        self, batch_data: List[Dict], accumulate_gradients: bool = False
    ) -> Dict:
        """
        CUDA加速的训练步骤
    ...
    def validate_cuda(self, val_data: List[Dict]) -> Dict:
        ...
        with torch.no_grad():
            for i in range(0, len(val_data), batch_size):
                ...
                src_mask, tgt_mask, tgt_padding_mask = self.create_masks(
                    src_batch, tgt_input
                )
    ...
    def train_epoch_cuda(
        self, train_data: List[Dict], val_data: List[Dict], epoch: int
    ) -> Dict:
        """
        CUDA加速的epoch训练
    ...
    def run_cuda_training(
        self, train_data: List[Dict], val_data: List[Dict], output_dir: str = "outputs"
    ) -> Dict:
        """
        运行完整的CUDA训练流程
        ...
        try:
            for epoch in range(epochs):
                ...
                logger.info(
                    f"  早停计数: {epoch_metrics['patience_counter']}/{self.early_stopping_patience}"
                )

                # 保存检查点
                if (epoch + 1) % save_frequency == 0 or epoch_metrics[
                    "val_loss"
                ] == self.best_val_loss:
                    checkpoint_path = os.path.join(
                        output_dir, f"cuda_checkpoint_epoch_{epoch + 1}.pth"
                    )
                    self.save_checkpoint(
                        epoch, epoch_metrics["val_loss"], checkpoint_path
                    )

                # 早停检查
                if epoch_metrics["early_stop"]:
                    logger.info(
                        f"🛑 早停触发 (patience: {self.early_stopping_patience})"
                    )
                    break
            ...
            self.save_checkpoint(
                self.current_epoch, self.best_val_loss, final_model_path
            )
