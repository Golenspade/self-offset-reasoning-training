...
class ReplayBuffer:
        ...
        for i in range(segments):
            ...
            if segment_data:
                segment_samples = random.sample(
                    segment_data, 
                    min(samples_per_segment, len(segment_data))
                )
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            'oldest_timestamp': min(exp['timestamp'] for exp in self.buffer) if self.buffer else None,
            'newest_timestamp': max(exp['timestamp'] for exp in self.buffer) if self.buffer else None
        }
...
class CumulativeLearningSystem:
    ...
    def __init__(self, replay_buffer: ReplayBuffer, config: Dict):
        ...
        # 学习参数
        self.new_data_ratio = config.get('new_data_ratio', 0.3)  # 新数据在训练批次中的比例
        self.training_iterations_per_loop = config.get('training_iterations_per_loop', 5)
    ...
    def prepare_training_batch(self, new_samples: List[Dict], batch_size: int) -> List[Dict]:
        """准备训练批次 - 新旧数据混合"""
...
def test_memory_system():
    ...
    # 添加一些测试经验
    test_experiences = [
        {'input_text': f'test_{i}', 'target_text': f'target_{i}', 'complexity_level': 'simple'}
        for i in range(50)
    ]
