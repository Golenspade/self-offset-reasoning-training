...
class TargetNetworkSystem:
    ...
    def soft_update(self):
        ...
        for target_param, learning_param in zip(
            ...
            self.learning_model.parameters()
        ):
            # 计算参数差异（用于监控稳定性）
        ...
        if self.update_counter % 100 == 0:
            logger.debug(f"目标网络软更新 #{self.update_counter}, 平均参数差异: {avg_divergence:.6f}")
        ...
        for input_seq in test_inputs[:10]:  # 只测试前10个样本
            try:
                ...
                if len(learning_pred) == len(target_pred):
                    matches = sum(1 for a, b in zip(learning_pred, target_pred) if a == b)
    ...
    def get_stability_report(self) -> Dict:
        ...
        return {
            ...
            'parameter_divergence': {
                ...
                'trend': 'stable' if np.std(param_div[-10:]) < np.std(param_div) else 'unstable'
            },
            'output_consistency': {
    ...
    def _compute_stability_score(self) -> float:
        ...
        # 参数稳定性：差异越小越稳定
        param_stability = 1.0 / (1.0 + np.mean(self.stability_metrics['parameter_divergence'][-10:]))
        
        # 输出一致性：一致性越高越稳定
        output_stability = np.mean(self.stability_metrics['output_consistency'][-5:]) if self.stability_metrics['output_consistency'] else 0.0
...
class StabilizedTrainingLoop:
    ...
    def train_step_with_stability(self, batch_data: List[Dict], tokenizer) -> Dict:
        ...
        if self.target_system.update_counter % self.stability_check_frequency == 0:
            ...
            consistency = self.target_system.evaluate_consistency(test_inputs, tokenizer)
            ...
            # 如果稳定性过低，考虑硬更新
            if stability_report.get('stability_score', 0.0) < self.min_stability_threshold:
                logger.warning(f"稳定性过低 ({stability_report.get('stability_score', 0.0):.3f})，考虑调整训练策略")
