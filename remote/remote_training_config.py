...
class RemoteTrainingConfig:
    ...
    def _load_base_config(self):
        ...
        self.remote_output_path = os.getenv(
            "REMOTE_OUTPUT_PATH", "/outputs/training_results"
        )
        self.remote_checkpoint_path = os.getenv(
        ...
        # ==================== 云存储配置 ====================
        self.cloud_provider = os.getenv(
            "CLOUD_PROVIDER", "local"
        )  # aws, gcp, azure, aliyun, local
        ...
        self.email_notifications = (
            os.getenv("EMAIL_NOTIFICATIONS", "false").lower() == "true"
        )
...
def create_default_config_file(
    output_path: str = "configs/remote_training_config.json",
):
    """创建默认的远程训练配置文件"""
    default_config = {
        "batch_size": 32,
