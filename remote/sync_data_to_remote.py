    ...
    def _upload_to_azure(self, local_path: str, remote_path: str) -> bool:
        ...
        try:
            blob_client = self.client.get_blob_client(
                container=self.config.cloud_bucket, blob=remote_path
            )
...
class DataSyncManager:
    ...
    def sync_training_data(self, force_regenerate: bool = False) -> bool:
        ...
        try:
            ...
            if (
                force_regenerate
                or not os.path.exists(local_train_data)
                or not os.path.exists(local_val_data)
            ):
            ...
            success &= self.storage_manager.upload_file(
                local_train_data, remote_train_path
            )
    ...
    def download_training_data(self) -> bool:
        ...
        try:
            ...
            local_train_path = os.path.join(
                self.config.remote_data_path, self.config.train_data_file
            )
            local_val_path = os.path.join(
                self.config.remote_data_path, self.config.val_data_file
            )

            success = True
            success &= self.storage_manager.download_file(
                remote_train_path, local_train_path
            )
            success &= self.storage_manager.download_file(
                remote_val_path, local_val_path
            )
...
def main():
    ...
    parser.add_argument(
        "--action",
        choices=["upload", "download", "sync-checkpoints"],
        required=True,
        help="操作类型",
    )
    parser.add_argument("--config", help="配置文件路径")
    parser.add_argument(
        "--force-regenerate", action="store_true", help="强制重新生成数据"
    )
    parser.add_argument(
        "--checkpoint-dir", default="outputs/checkpoints", help="检查点目录"
    )
