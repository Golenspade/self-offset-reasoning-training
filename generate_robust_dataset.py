    ...
    while successful_samples < num_samples and attempts < max_attempts:
        ...
        try:
            ...
            if (sample['noisy_prop'].strip() and 
                ...
                sample['noisy_prop'] != sample['target_contrapositive'] and
                len(sample['noisy_prop']) > 5):
...
def analyze_robust_dataset(dataset: list, name: str):
    ...
    print(f"  长度范围: 输入({min(input_lengths)}-{max(input_lengths)}), 目标({min(target_lengths)}-{max(target_lengths)})")
